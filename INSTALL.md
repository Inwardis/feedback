...
3. **Create a `.env` file** next to them with the three required secrets (below) **and the
   ...
   ```
   INWARDIS_VERSION=1.0.0-rc.76   # ← the version from your download page / bundle filename
   ```
...
|----------|---------|---------|
| `INWARDIS_VERSION` | **required** | The version of your downloaded bundle (e.g. `1.0.0-rc.76`). The compose file pins all three image tags by it and refuses to start without it — a wrong value fails with *image not found* instead of silently running an older image. Verify at `/actuator/info`. |
