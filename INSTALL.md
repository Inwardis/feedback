...
3. **Create a `.env` file** next to them with the three required secrets (below) **and the
   ...
   ```
   INWARDIS_VERSION=1.0.0-rc.70   # ← the version from your download page / bundle filename
   ```
...
|----------|---------|---------|
| `INWARDIS_VERSION` | **required** | The version of your downloaded bundle (e.g. `1.0.0-rc.70`). The compose file pins all three image tags by it and refuses to start without it — a wrong value fails with *image not found* instead of silently running an older image. Verify at `/actuator/info`. |
...
| `INWARDIS_AGENT_COMMITS` | `true` | Whether an AI agent's changes become versions on their own — grouped per project and named after the agent. `false` leaves versioning to saves and explicit checkpoints |
| `INWARDIS_AGENT_COMMITS_QUIET` | `5m` | How long an agent must be quiet before its changes are cut into a version |
| `INWARDIS_AGENT_COMMITS_MAX_WINDOW` | `30m` | The longest an agent's change may stay unversioned while it keeps working |
